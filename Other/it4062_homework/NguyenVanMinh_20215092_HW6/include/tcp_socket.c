...
bool send_msg(char* message, int connfd) {
  ...
  if (msglen > MSG_SIZE - DELIMITER_SIZE) {
    fprintf(stderr, "send_msg(): message lenght must be less than %ld character\n", MSG_SIZE - DELIMITER_SIZE);
    return false;
  }
...
int get_msg(int connfd, char* message, char* recv_buffer, int timeout) {
  ...
  do {
    if (recv_buffer[0] != '\0') { // get residual messages from recv_buffer
      ...
      if (received_bytes < 0) {
        if (errno == ECONNRESET) return -2; // connection closed
        else {
          perror("\nrecv()");
          return -1;
        }
    ...
    while (1) {
      if (ptr == NULL) {
        if ( strlen(recv_msg) + strlen(optr) > MSG_SIZE - 1) return -3; // message overlong
